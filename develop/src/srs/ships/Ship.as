					...
					if (situation.target.ship != null)
					{
						...
						if (torp_params != null)
						{
							...
							if (cur_torp_app != null)
							{
								...
								var vp:VehicleParams = new VehicleParams(situation.target.ship.position_gm, situation.target.ship.velocity_gm, situation.target.ship.direction_deg);
								...
								for (var i:int = 1; i < Settings.AI_torped_fire_interval; i++)
								{
									...
									var p:Point = Point.polar(t_ms * situation.target.ship.getVelocity(), Utils.toScreenRad(situation.target.ship.getDirection()));
									...
									vp = VehicleMoving.move_calc_pos2(dt, vp, situation.target.ship.getCommandParams(), situation.target.ship.max_velocity_hum, situation.target.ship.manevr_prc);
...
//main.registerEnemyTorped(torp);
		...
		protected function isTorpedReady():Boolean
		{
			...
			return (getTimer() - time_torp_fire) > time_reload_torped_ms + Math.random() * time_reload_torped_ms / 2.;
			//-- выход
