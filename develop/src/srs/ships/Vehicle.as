...
public class Vehicle extends Sprite {
	...
	protected static const ST_MOVE_IN_DIRECTION:int = 10;
	...
	protected var follow_direction_deg:Number = 0. ;  //-- следовать в направлении 
	...
	public function AI_fast_loop():void {
		...
		//-- Команды есть? и//-- состояние движения НАЧАЛО ДВИЖЕНИЯ ПО ПРОГРАММЕ или ДВИЖЕНИЕ ПО ПРОГРАММЕ?
		if(commands != null 
			&& (move_state == ST_BEG_COMMAND_MOVING || move_state == ST_COMMAND_MOVING)) {
			//-- движение по командам (программе)
			...
			//-- состояние движения  ДВИЖЕНИЕ В НАПРАВЛЕНИИ?
			if(move_state == ST_MOVE_IN_DIRECTION) {
				//-- вычисляем рассогласование направления на точку и направления объекта
				var delta:Number = direction_deg - follow_direction_deg;
 
				//-- отработка рассогласования курса//-- 
				calcRudderOnDeltaAngle(delta); 
			} else {
				//-- состояние движения  не НЕИЗВЕСТНО и не МТ_ДОСТИГНУТА?
				if(move_state != ST_MOVE_UNKNOWN && move_state != ST_WP_FINISHED) {
...
if(current_way_point == way_points.length -1) {
	...
	//-- calcRudderOnDeltaAngle()
	public function  calcRudderOnDeltaAngle(delta:Number):void { 
		//-- Устанавливаем руль в соответствии с отклонением от направления. При больших отклонениях выше степень оклонения.//-- 
	...
	public function move():void { 
		//-- Движение. Отрабатывает установленные руль и power.//-- Не работает если игра остановлена или прошло меньше 50 мс с прошлого вызова.
				if (!main.isStarted()) {
			return;
		}
		//before_move();
	...
	public function AI_step_II():void {}
		...
		//-- addWayPoint()//-- startMoveOnWayPoint()
			/*
	...
	protected function onWpFineshed():void {
		...
		//-- startMoveInDirectionDeg()
					/*
			 * Активирует движение в заданном направлении
			 * 
			 */ 
			protected function startMoveInDirectionDeg(dir_deg:Number):void {
					move_state = ST_MOVE_IN_DIRECTION;
					setFollowDirectionDeg(dir_deg);
			}
 
		//-- addCommand()//-- setCommandMoveState()
			/*
	...
	protected function removeTail():void {
		...
		//-- setMaxVelocityHum(),getMaxVelocityHum(),//-- moveOnTarget(),selectTarget(),drawShip(),
	...
	public function getTimeLiveMs():uint {
		...
		//-- setDirection() град.//-- setFollowDirectionDeg()
			...
			public function setFollowDirectionDeg(dir_deg:Number):void {
				this.follow_direction_deg = dir_deg;	
			}
 
		//-- setForces(),getForces(),setPower(),getPower(),//-- setRudder(),start_move(),setName(),
	...
	public function setPosition2(p:Point):void {
		...
		//-- getPhisVelocity() - phisical velosity in knots//-- getVelocity()
			/**
	 * 
	...
	public function set_velocity(v:Number):void {
		...
		//-- get/set/is UnderControl//-- get/set Manevr
